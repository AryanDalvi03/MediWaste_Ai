const TeamAnalytics = () => {
  ...
  return (
    <div className="space-y-8 animate-slide-up">
      <header>
        <h2 className="text-4xl font-extrabold tracking-tight text-foreground">Team Analytics</h2>
      ...
      <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
        ...
        <div className="space-y-3">
          {staff.map((s) => (
            <div key={s.name} className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border/50">
              <div className="flex items-center gap-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                  s.rank === 1 ? 'bg-warning-light text-warning-foreground' : 'bg-muted text-muted-foreground'
                }`}>{s.rank}</div>
                <div>
              ...
              <div className="text-right">
                <p className="font-bold text-primary">{s.accuracy}%</p>
...
const StatCard = ({ label, value, sub, accent, warning }: { label: string; value: string; sub?: string; accent?: boolean; warning?: boolean }) => (
  <div className={`p-6 rounded-2xl border shadow-sm ${warning ? 'bg-warning-light border-warning/20' : 'bg-card border-border'}`}>
    <p className={`text-xs font-bold uppercase tracking-wider mb-2 ${warning ? 'text-warning-foreground' : 'text-muted-foreground'}`}>{label}</p>
    <div className={`text-3xl font-extrabold tracking-tight ${accent ? 'text-primary' : warning ? 'text-warning-foreground' : 'text-foreground'}`}>{value}</div>
