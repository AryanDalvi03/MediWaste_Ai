...
import {
  ...
  Users, ClipboardCheck, Building2, LogOut, Zap, ChevronLeft, ChevronRight, ShieldCheck, Sun, Moon, BarChart3, Briefcase
} from 'lucide-react';
...
const hospitalItems = [
  ...
  { id: 'mywork' as const, icon: Briefcase, label: 'My Work' },
...
const Sidebar = () => {
  ...
  const filteredNavItems = navItems.filter((item) => {
    ...
    // hospital_staff does NOT see ESG Impact
    if (item.id === 'green' && role === 'hospital_staff') return false;
    return true;
  });

  const showHospital = role === 'audit_manager' || role === 'hospital_staff';

  // audit_manager sees team + compliance + facility
  // hospital_staff sees only 'My Work'
  const filteredHospitalItems = hospitalItems.filter((item) => {
    if (role === 'hospital_staff') return item.id === 'mywork';
    if (role === 'audit_manager') return item.id !== 'mywork'; // admin doesn't need My Work
    return false;
  });
