...
interface SmartBin {
  ...
  status: 'Active' | 'Full' | 'Maintenance' | 'Collected' | 'Disposed';
  overallFill: number;
  worker: string;
  workerRole: string;
...
const BinCard = ({ bin, onRemove, onAssign, staffList }: { bin: SmartBin, onRemove: (id: string) => void, onAssign: (binId: string, worker: string) => void, staffList: any[] }) => {
  const isDanger = bin.overallFill >= 85;
  const [isEditingAssignee, setIsEditingAssignee] = useState(false);
  const [newAssignee, setNewAssignee] = useState(bin.worker);
  ...
  return (
    ...
    >
      ...
      {/* Worker */}
      <div className="flex items-center gap-2 pt-2 border-t border-border/20 z-10 relative">
          ...
          {isEditingAssignee ? (
            <div className="flex items-center gap-2">
              <select 
                ...
                className="bg-background/80 text-foreground text-xs px-2 py-1 rounded w-full outline-none font-bold"
              >
                {Array.isArray(staffList) && staffList.filter(s => s.floor === bin.floor).map(s => (
                  <option key={s._id} value={s.name}>{s.name} ({s.role})</option>
                ))}
                {Array.isArray(staffList) && staffList.filter(s => s.floor !== bin.floor).map(s => (
                  <option key={s._id} value={s.name}>{s.name} (Floor {s.floor})</option>
                ))}
              </select>
              <button 
                onClick={() => { onAssign(bin.id, newAssignee); setIsEditingAssignee(false); }}
                className="text-primary hover:text-primary-foreground bg-primary/10 hover:bg-primary px-2 py-1 rounded transition-all text-[10px] font-bold"
              >
                Save
              </button>
            </div>
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsEditingAssignee(true)} className="text-muted-foreground hover:text-primary transition-colors p-1">
...
const BinOperations = () => {
  ...
  const fetchStaff = () => {
    fetch(`${apiBase}/api/staff`)
      ...
      .then(data => {
        ...
        else console.warn("Expected array, received:", data);
      })
      .catch(console.error);
  };
  ...
  const fetchLogs = () => {
    fetch(`${apiBase}/api/work_log?limit=30`)
      .then(res => res.json())
      .then(data => { if (Array.isArray(data)) setWorkLogs(data); })
      .catch(console.error);
  };
  ...
  const removeBin = async (binId: string) => {
    try {
        await fetch(`${apiBase}/api/bins/${binId}`, { method: 'DELETE' });
        fetchBins();
    } catch(e) { console.error(e) }
  };

  const assignBin = async (binId: string, worker: string) => {
    try {
        await fetch(`${apiBase}/api/bins/${binId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedWard.bins.map((bin) => (
                    <BinCard key={bin.id} bin={bin} onRemove={removeBin} onAssign={assignBin} staffList={staffList} />
                  ))}
                </div>
              </div>
            )}

            {/* ── Staff Work Log Audit Trail (admin only) ── */}
            {workLogs.length > 0 && (
              <div className="glass-card p-6 rounded-2xl mt-6">
                <div className="flex items-center gap-3 mb-4 border-b border-border/20 pb-3">
                  ...
                  <h3 className="font-bold text-base text-foreground">Staff Activity — Audit Trail</h3>
                  <span className="ml-auto text-[10px] font-display font-bold text-muted-foreground uppercase tracking-widest">{workLogs.length} entries</span>
                </div>
                <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
                  {workLogs.map(log => {
                    ...
                    const meta = actionMeta[log.action] || { label: log.action, color: 'text-muted-foreground', Icon: AlertTriangle };
                    return (
                      <div key={log._id} className="flex items-start gap-3 p-3 rounded-xl glass-card-hover">
                        <meta.Icon className={`w-4 h-4 shrink-0 mt-0.5 ${meta.color}`} />
