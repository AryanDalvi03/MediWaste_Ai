...
export function AIScanner() {
  ...
  const downloadReport = () => {
    ...
    const report = [
      '═══════════════════════════════════════════',
      '       MEDIWASTE AI — SCAN REPORT',
      '═══════════════════════════════════════════',
      '',
      `Scan Date & Time : ${result.timestamp}`,
      '',
      '───────────────────────────────────────────',
      '  CLASSIFICATION RESULT',
      '───────────────────────────────────────────',
      `  Waste Class      : ${result.class}`,
      `  Confidence        : ${result.confidence.toFixed(2)}%`,
      `  Disposal Bin      : ${result.disposal_bin}`,
      `  Hazard Status     : ${result.hazard_status}`,
      '',
      '───────────────────────────────────────────',
      '  DISPOSAL INSTRUCTIONS',
      '───────────────────────────────────────────',
      result.hazard_status === 'Hazardous'
        ? '  ⚠  Handle with PPE. Use designated\n     hazardous waste bin. Follow facility\n     biohazard protocols.'
        : '  ✓  Standard disposal protocol applies.\n     Segregate appropriately.',
      '',
      '═══════════════════════════════════════════',
      '  Generated by MediWaste AI Scanner',
      '═══════════════════════════════════════════',
    ].join('\n');
    ...
    a.download = `MediWaste_Scan_${ts}.txt`;
    a.click();
        ...
        <CardContent className="space-y-4">
          ...
          {result && (
            <div className="mt-6 space-y-4 border rounded-lg p-4 bg-background">
              ...
              <div className="text-center border-t pt-4 mt-4 space-y-3">
